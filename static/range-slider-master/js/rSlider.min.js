!function () {
    ...
    t.prototype.createScale = function (t) {
        ...
        for (var e = 0, s = this.conf.values.length; e < s; e++) {
            ...
            n.appendChild(l), this.scale.appendChild(n),
            n.style.width = e === s - 1 ? 0 : this.step + "px",
           this.conf.labels && (this.conf.values[e] >= 1900 && this.conf.values[e] % 20 === 0)
...
t.prototype.updateScale = function (newValues) {


    // Update the values in the configuration
    this.conf.values = newValues;
    ...
    // Redraw the scale based on the new values
    for (var e = 0, s = this.conf.values.length; e < s; e++) {
        var n = i("span"),
            l = i("ins");
        n.appendChild(l);
    ...
    // Update the pointer positions based on the new scale
    ...
    t.prototype.updateValuesRange = function(newValues) {
    // Update the values in the configuration
    this.conf.values = newValues;

    // Clear the existing scale and selected values
    this.scale.innerHTML = '';
    ...
    // Update the slider scale and UI
    this.updateScale();

    // Set new values and position pointers
    this.setValues();
};
